            ...
            <div class="page-header-content pt-4">
                <div class="row align-items-center justify-content-between">
                    ...
                    <div class="col-auto my-4">
                        ...
                        <a href="{{ route('list-gudang.index') }}" class="btn btn-danger add-list"><i
                ...
                <nav class="mt-4 rounded" aria-label="breadcrumb">
                    <ol class="breadcrumb px-3 py-2 rounded mb-0">
                        ...
                        <li class="breadcrumb-item active">List Data Gudang</li>
                    </ol>
                </nav>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="row mx-n4">
                    <div class="col-lg-12 card-header mt-n4">
                        <form action="{{ route('list-gudang.index') }}" method="GET">
                    ...
                    <div class="col-lg-12">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    @forelse ($listgudang as $listgudangs)
                                        <tr>
                                            <th scope="row">
                                                {{ $listgudang->currentPage() * (request('row') ? request('row') : 10) - (request('row') ? request('row') : 10) + $loop->iteration }}
                                            </th>
                                            <td>{{ $listgudangs->name }}</td>
                                            <td>{{ $listgudangs->email }}</td>
                                            <td>{{ $listgudangs->phone }}</td>
                                            <td>
                                                @if ($listgudangs->status_id == 1)
                                                    Aktif
                                                @else
                                                    Tidak Aktif
                                                @endif
                                            ...
                                            <td>
                                                @if ($listgudangs->status_id == 1)
                                                    <form id="status-form-{{ $listgudangs->id }}"
                                                        action="{{ route('change-status-gudang', $listgudangs->id) }}"
                                                        method="POST">
                                                        @method('PUT')
                                                        @csrf
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input" name="status" type="checkbox"
                                                                role="switch"
                                                                id="flexSwitchCheckChecked-{{ $listgudangs->id }}" checked
                                                                onchange="submitForm({{ $listgudangs->id }})">
                                                        </div>
                                                    </form>
                                                @else
                                                    <form id="status-form-{{ $listgudangs->id }}"
                                                        ...
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input" name="status" type="checkbox"
                                                                role="switch"
                                                                id="flexSwitchCheckChecked-{{ $listgudangs->id }}"
                                                                onchange="submitForm({{ $listgudangs->id }})">
                                                        </div>
                                                    </form>
                                                @endif
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{ route('list-gudang.edit', $listgudangs->id) }}"
                                                        class="btn btn-outline-primary btn-sm mx-1"><i
                                                            class="fas fa-edit"></i></a>
                                                    <form action="{{ route('list-gudang.destroy', $listgudangs->id) }}"
                                                        method="POST">
                                                        @method('delete')
                                                        @csrf
