...
@section('content')
            ...
            <div class="page-header-content pt-4">
                ...
                <nav class="mt-4 rounded" aria-label="breadcrumb">
                    <ol class="breadcrumb px-3 py-2 rounded mb-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('products-toko.index') }}">Produks</a></li>
    ...
    <div class="container-xl px-2 mt-n10">
        <form action="{{ route('products-toko.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-xl-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control form-control-solid @error('product_code') is-invalid @enderror"
                                    ...
                                    value="{{ old('product_code', $product->product_code) }}" autocomplete="off" />
                                @error('product_code')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                                        ...
                                        @if ($grades && $grades->first() && $grades->first()->detailValues->isNotEmpty())
                                            @foreach ($grades->first()->detailValues as $detail)
                                                ...
                                                <option value="{{ $detail->id }}"
                                                    {{ optional($product)->grade_id == $detail->id ? 'selected' : '' }}>
                                                    {{ $detail->name }}
                                                </option>
                                            @endforeach
                                        @else
                            ...
                            <div class="row gx-3 mb-3">
                                ...
                                <div class="col-md-6">
                                    <label class="small mb-1" for="gudang_id">Supplier <span
                                            class="text-danger">*</span></label>
                                    <select class="form-select form-control-solid" id="gudang_id" name="gudang_id">
                                        @if ($supplier->isNotEmpty())
                                            <option value="">Pilih Tipe Supllier</option>
                                            @foreach ($supplier as $detail)
                                                <option value="{{ $detail->id }}"
                                                    {{ optional($product)->supplier_id == $detail->id ? 'selected' : '' }}>
                                                    {{ $detail->name }}
                                                </option>
                                            @endforeach
                                        @else
                                    ...
                                    <select class="form-select form-control-solid" id="mades_id" name="made_in_id">
                                        @if ($mades && $mades->first() && $mades->first()->detailValues->isNotEmpty())
                                            @foreach ($mades->first()->detailValues as $detail)
                                                ...
                                                <option value="{{ $detail->id }}"
                                                    {{ optional($product)->made_in_id == $detail->id ? 'selected' : '' }}>
                                                    {{ $detail->name }}
                                                </option>
                                            @endforeach
                                        @else
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control form-control-solid @error('detail') is-invalid @enderror" id="detail"
                                    name="detail" rows="3">{{ old('detail', $product->detail) }}</textarea>
                                @error('detail')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <a class="btn btn-danger" href="{{ route('products-toko.index') }}">Cancel</a>
                        </div>
                    </div>
                    <!-- END: Produk Details -->
                </div>
