                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @if ($purchase->purchase_status == 0)
                            <form action="{{ route('purchases.returnProduct') }}" method="POST">
                                @csrf
                        ...
                        @else
                            ...
                            <a class="btn btn-danger" href="{{ route('purchases.invoiceBill', $purchase->id) }}">Retur</a>
                        @endif
                        @if ($purchase->arrival_date != null)
                            <a class="btn btn-success"
                                href="{{ route('purchases.invoiceBill', $purchase->id) }}">Tagihan</a>
                        @endif
                    </div>
                </div>
            </div>
                            ...
                            <tbody>
                                @foreach ($purchaseDetails as $item)
                                    <tr>
                                        ...
                                        <td scope="row">
                                            <span class="btn btn-warning">{{ $item->productToko->stock }}</span>
                                        </td>
                                        <td scope="row">
                                        ...
                                        <input type="hidden" name="product_id[{{ $item->productToko->id }}]"
                                            value="{{ $item->product->id }}">
                                        <input type="hidden" name="unit_cost[{{ $item->productToko->id }}]"
                                            value="{{ $item->unitcost }}">
                                        <input type="hidden" name="total_amount[{{ $item->productToko->id }}]"
                                            value="{{ $item->total }}">
                                        <td>
                                            <input type="number" name="return_quantity[{{ $item->productToko->id }}]"
                                                class="form-control form-control-solid" min="0"
                                                max="{{ $item->quantity }}" step="1">
                                        </td>
                                        <td>
                                            <textarea name="return_reason[{{ $item->productToko->id }}]" class="form-control form-control-solid"></textarea>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
