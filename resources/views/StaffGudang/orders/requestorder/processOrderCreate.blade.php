        ...
        <div class="container">
            <div class="row">
                                ...
                                <table class="default-table invoice-table">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><strong>{{ $countUnitcost }}</strong></td>
                                        </tr>
                                        <tr>
                                            <td colspan="3" class="text-end"><strong>PPN 12%</strong></td>
                                            <td><strong>{{ $ppnCount }}</strong></td>
                                        </tr>
                                        <tr>
                                            <td colspan="3" class="text-end"><strong>Total</strong></td>
                                            <td><strong>{{ $ppnResult }}</strong></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <div class="invoice-btn-section clearfix d-print-none">
                    <a class="btn btn-lg btn-primary" href="{{ route('processOrder', $order_id) }}">
                        Back
                    </a>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title text-center mx-auto" id="modalCenterTitle">Invoice of
                        {{ $customer->name }}<br />Total Amount Rp {{ $ppnResult }}</h3>
                </div>

                <form action="{{ route('processOrder.createOrder') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="modal-body">
                            <input type="hidden" name="customer_id" value="{{ $customer->id }}">
                            <input type="hidden" name="agen_id" value="{{ $sales->id }}">
                            <input type="hidden" name="total_products" value="{{ $count }}">
                            <input type="hidden" name="sub_total" value="{{ $countUnitcost }}">
                            <input type="hidden" name="total" value="{{ $ppnResult }}">
                            <input type="hidden" name="vat" value="{{ $ppnCount }}">
